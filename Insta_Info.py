try:
    import requests,re,time,os,pyfiglet,termcolor
...
def check_username(username):
    ...
    if len(word) == 1:
        ...
        if i.status_code == 404:
            return '['+termcolor.colored('!','red')+'] '"The username does NOT EXIST!\n"
    ...
    if datas != 0:
        ...
        try:
            ...
            combo = f'''
----- Info -----
{termcolor.colored('Email','cyan')}: {maily}
{termcolor.colored('Number','cyan')}: {numb}
...
{termcolor.colored('Bio','cyan')}: {bio}
{termcolor.colored('Other Mails In The Page','cyan')}: {mails}
{termcolor.colored('Last Post Date','cyan')}: {date}
{termcolor.colored('Followers','cyan')}: {cv[0].split(' ')[0]}
{termcolor.colored('Following','cyan')}: {cv[1].split(' ')[0]}
{termcolor.colored('Posts','cyan')}: {cv[2].split(' ')[0]}
{termcolor.colored('Time','cyan')}: {time.asctime()}
----------------
'''         
            return combo
            #return i.text
...
print(termcolor.colored('                       CopyRight >> instagram.com/a7.cc','magenta'))


print()
print(cyany+'Welcome to Insta Info v1 !')
print(cyany+"This program gives you information about a certain Instagram account just by typing the account's username.")
print()
...
while True:
    def do():
        ...
        print(info)
    do()
    print('('+blue('1')+') Check another username.')
    print('('+blue('2')+') Exit')
    ans = input()
    if ans == str('2'):
        break
